...
2. **Intake check**: if `data/raw/CASE_XXX/` doesn't exist yet, run intake first (D2 — `_source_ledger.json` gate, every file `pending`→human sets `approved`/`rejected`, whole case blocks on any rejection). Never skip the human confirmation step.
   - **Segmentation check**: a raw source may be a *bundle* concatenating several logical documents. The orchestrator/operator currently identifies which approved raw PDFs are bundles; automatic bundle-candidate detection is not implemented. Before stage 2, split every identified bundle into per-document `DOC_XXX.pdf` via `tools/segment_case.py` (`sheets`/`propose [--refine]`/`approve`/`split`). `propose` automatically rechecks crop-ambiguous `needs_full_page` pages full-page when the 25% spend cap is not saturated; `--refine` is a separate, opt-in pass over long segments that may have been confidently over-merged. Both use the owner-set title rule: own title → new document, no title after full-page inspection → continuation, unreadable → human review. This has its **own human-review gate**: `split` refuses until a human approves the `segmentation_proposal_{DOC}.json` (case-level `approved` AND every segment `approved`/`edited` AND no unassigned page). Do not dispatch stage 2 on an unsplit identified bundle, and never approve the proposal on the model's behalf. Records under the `document_segmentation` stage. No OCR happens here — segmentation output is document structure, not text; `document-pipeline` still owns real classification.
