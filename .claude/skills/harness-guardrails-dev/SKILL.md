...
2. **Only after the screening report has been signed off** — `dao.py mark-human-review-complete CASE_ID screening`, which requires the report to exist and its stage to have passed. Enforced by the commands; they refuse without the flag. **The review token must match the artifact being scored**: `screening` authorizes `screening_fidelity` only, and v1/v2 (draft reviews, gated on `expert_review_v{n}.json`) authorize `evaluation` only. Reusing the draft flag here was a real defect — a case that stops at `screening_report` never produces a draft review, so the gate could only have been opened by fabricating one.
