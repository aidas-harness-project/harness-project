...
- Provider-based P8 does not automatically mean genuinely different extraction technologies. Same-provider runs such as `openai-api` + `openai-api` or `claude-cli` + `claude-cli` reduce one-off transient misreads, but they still share systematic blind spots. Even two LLM-vision providers are weaker than a traditional OCR engine plus an independent vision read. See `open-decisions.md`.
