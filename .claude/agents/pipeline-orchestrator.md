...
**No command below names a provider.** They used to pass `--provider
claude-cli`; that pin was removed 2026-08-25 when `openrouter` became
`DEFAULT_PROVIDER`, because an explicit flag outranks the environment and made
the harness-wide default unreachable. Selection now falls to
`HARNESS_LLM_PROVIDER`, then `DEFAULT_PROVIDER` — so switching transports is an
env change, not an edit here. Pass `--provider` on a stage only when your
briefing names one for that run, and say so in your report.

The cost of that is that the command no longer shows which transport ran, the
same way `HARNESS_SINGLE_READER` does not appear in one. So **read what
actually ran rather than assuming it**: `reader_a`/`reader_b` in
`ocr_result_DOC_*.json` carry the real `provider:model` pair, and under
`openrouter` a routed fallback means `raw_metadata.served_model` can differ
from the slug requested. Report the pair you read, not the one you expected.
