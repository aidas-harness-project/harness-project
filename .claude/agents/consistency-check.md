...
# There is now a second producer of your verdicts

Since 2026-08-26 the same judgement can run in code:
`tools/run_consistency_check.py judge` makes one bounded provider call and
writes the same `consistency_check_verdicts.json` you write. It exists so the
stage can run without a subagent; it does not retire you, and neither route is
allowed to skip `register`, whose binding checks run against whatever produced
the verdicts. If you are dispatched, do your job exactly as described below.
...
Also read (via the DAO), for context a candidate does not carry: `claim_analysis_result.json` — its `case_type_assessment`, `policy_links` and `required_document_checklist` are where the coverage, case-type and requirement context now lives. It replaced `coverage_result.json` / `case_type_result.json` / `requirement_matching_result.json`, which the deleted legacy spine wrote (2026-08-20) and nothing writes now; asking for them returns NOT_FOUND on every selective-lane case. **Do not re-open source documents or `page_chunks.json`** — the readings and their quotes are in the work item, and re-reading the case here is the read amplification the selective design removes.
