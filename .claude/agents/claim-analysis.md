...
**Checkpoint 2 — Coverage Identification.** Policy text (from `policy-pipeline`) + claim fields → `coverage_result.json` — an array under `coverages`, since a claim can trigger more than one coverage at once. Per coverage: `coverage_name` + `standardized_coverage_name`, `applicable` (whether its conditions are actually met by the claim's facts, not just whether the claim mentions it), `matched_clause_ref` (`{document_id, clause_id}` — `document_id` picks which `normalized_policy_clause_{document_id}.json` file, `clause_id` picks the clause within it, since policy-pipeline writes one file per policy document, not one combined file), confidence + evidence.
