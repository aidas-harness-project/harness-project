...
After checkpoint 1, derive the evidence-grounded medical-variable content only from the validated redacted evidence, but do not publish it yet. Canonical publication validates the candidate against `case_type_result.json`, which does not exist until checkpoint 3. Keep `case_type` unset until that checkpoint rather than guessing it early.
...
lists every article heading in the case's policy documents with the page it
sits on and the 약관 that owns it, plus any table whose row/column structure
was recovered from the PDF. A 323-page policy bundle yields ~564 headings, so
...
**Checkpoint 3 — Case Type and Report Profile Classification.** Claim fields + coverage → `case_type_result.json`, including `report_profile` and `template_id`.
...
The two broad axes do not by themselves identify a report form. Set `report_profile` from the actual contractual mechanism and assignment:

- third-party automobile bodily-injury compensation → `automobile_compensation` / `statutory_or_policy_auto_compensation` / `compact` / `supported`, template `자동차보험_대인배상_간이형`;
- first-party automobile self-injury benefit → `automobile_self_injury` / `automobile_policy_benefit` / `full` / `provisional`, template `자동차보험_자기신체사고형`, with `review_required: true` and `reviewer_role: "손해사정사"` because the corpus has one example;
- first-party personal-accident disability benefit → `personal_accident_benefit` / `personal_accident_policy_benefit` / `full` / `supported`, template `개인보험_후유장해형`;
- third-party liability damages → `liability_damages` / the evidenced `insured_liability` or `mutual_or_cooperative_liability` mechanism / `full` / `supported`, template `배상책임_후유장해형`;
- disease/diagnosis-triggered benefit → `disease_benefit` / `disease_policy_benefit` / `full` / `supported`, template `진단수술비형`.

For `실손`, an unconfirmed primary type, a non-claim case, or any assignment with no supported/provisional registry contract, set family and mechanism to `other_review_required`, `support_status: unsupported`, `template_id: null`, and `review_required: true`. Do not borrow another family's template. The draft stage must halt on this profile and report what representative material is missing.

For supported/provisional profiles, `template_id` must be the matching canonical key from `templates/registry.json`. DAO validation cross-checks family, mechanism, mode, and support status against that registry and refuses a mismatch. `case_type_result.json` still records the legacy primary `case_type`, optional genuine `secondary_case_types`, and auditable Top-N `candidate_types`.

After checkpoint 3 has produced canonical `case_type_result.json`, publish the medical variables. Copy its exact `case_type` into the completed candidate before calling the canonical publication boundary:

```bash
python tools/dao.py write-medical-variables CASE_ID DATA_FILE --held-by claim-analysis --run-id RUN_ID
```

This publication structures evidence; it does not make a clinical inference, choose a referral policy, or invent human action. If the configured medical policy is disabled or the candidate is rejected, halt at checkpoint 3.
...
Upstream: `document-pipeline` (redacted/chunked text), `policy-pipeline` (normalized clauses). Downstream: `consistency-check` (cross-references your output against other sources), `screening-report` (case overview, `report_profile`, `template_id`).
