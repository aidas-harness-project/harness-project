...
# There is now a second producer of your contract

Since 2026-08-26 the authoring can run in code: `tools/run_draft_report.py`
writes the same `loss_adjustment_report_{version}.json` you write, one block at
a time, and `document_assembly.py` renders it afterwards exactly as it renders
yours. On that path statements cite `E<N>` ids into a registry the driver builds,
so a document id, a page or a quote cannot be written by a model at all. It does
not retire you, and it never overwrites a version's artifacts. If you are
dispatched, do your job exactly as described below.
...
For both versions, first inspect `extracted_claim_fields.json` through the DAO. Only a DAO-accepted projection that explicitly declares `projection_mode: legacy_pre_medical` is pre-adoption legacy mode: do not run the medical gate/outcome commands, state that medical screening was unavailable, and never claim medical clearance. A missing or unknown mode is an error, not legacy. For `canonical_medical_projection`, run `python tools/dao.py check-medical-reviews-clear CASE_ID` and consume `python tools/dao.py read-medical-review-outcomes CASE_ID --caller-stage draft_report_v1 --run-id RUN_ID` for v1 or `python tools/dao.py read-medical-review-outcomes CASE_ID --caller-stage draft_report_v2 --run-id RUN_ID` for v2 before drafting. Preserve decision authority, assignment role-policy provenance, and the current response's complete authenticated attribution, interpretation, uncertainty, alternatives, and downstream-adjustment advice in every medically affected section. Never read the internal medical-review ledger directly or silently convert a qualified medical opinion into an unqualified report assertion.
