...
A case may populate both sections. Never collapse two reason lists into one main classification. Set a total amount only when every amount needed for that total is explicitly stated; otherwise use `null`. The deprecated `main_reason_*` and flat `reduction_amount` fields are not authoritative for new consumers.
