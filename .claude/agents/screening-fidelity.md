...
2. **The report's own run must have finished** — it exists and `screening_report` is `passed`. Pass `--version screening` everywhere; the draft tokens v1/v2 are refused for you. **No one has to have read the report first.** You measure pipeline performance, and a reader's presence says nothing about that. What the condition stops is you scoring a report whose run failed, which would be a number about nothing.
...
4. **F1 — field comparisons.** The denominator is not yours to pick. Read `config/claim_analysis/claim_analysis_routing_v0.1.json` and compare by `field_id`; record `routing_config_version` so the score says which universe it was taken against.
   - **Grade** comes from whichever axis the field declares — `medical_advisory_grade` for a medical domain, `priority_grade` for a `legal_factual` one. The config enforces exactly one of the two, so read it the way `selection.field_grade()` does and never merge the axes. 과실비율 and 적용 법조 are grade-A legal fields, not discretionary asides.
   - **`core_field` mirrors `critical_conflict_field`** — the config's own 32 "fields whose disagreement can change a determination". Do not keep a list of your own.
   - **A `deferred` field is `not_applicable`**, not a miss: the pipeline is not asked to extract it, so the answer key having a value there costs the report nothing.
   - **Copy the upstream account of an absence**: `pipeline_resolution_status` and `pipeline_unavailable_reason` from `claim_analysis_result.json`. A value absent because the document kind is missing, one the document had but left blank, and one never mentioned are three different failures.
   - **A `legal_opinion` field is not a records contradiction.** Two opposing 법률의견서 both belong in the file; a report that preserved both and declined to choose followed the rule.
...
6. **F3 — required documents.** The denominator is `required_documents_by_case_type` for the case's determined types, in the config's own `document_kinds` vocabulary. `correct` when the report called a relied-on kind 보유, `under` when it called it 미확인/부족, `miss` when it never named it. `over` (flagged as missing but never needed) is recorded and **not** counted against the score.
   - **Anything the answer key relied on that has no `field_id` and no `document_kind` goes to `out_of_universe_items`** — recorded, never scored. Binding the denominators to the pipeline otherwise leaves you able to measure only what the pipeline already knows to look for, and 사고경위서 is the standing example: the answer key's evidence for the accident narrative, in no `document_kind` at all.
