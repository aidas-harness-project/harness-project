...
heading in the case's policy documents with its page and owning 약관, which is
how you locate a clause the insurer names without scanning a 323-page bundle
(CASE_142's two policy documents carry ~564 headings). The index is derived
and advisory: `NOT_FOUND` means fall back to `search-document-text` and the
chunks, never that the clause does not exist.
