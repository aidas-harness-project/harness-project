...
- `read-page-text` is yours to use, because you own checkpoint 2 — it requires `--caller-stage document-pipeline` and the DAO refuses every other stage, so you are the only caller that gets text back. Be clear about what it hands you: `page_NNN.md` is checkpoint 1 output, **before redaction**, still carrying claimant-facing PII (names, addresses, phone numbers). It is redaction's input, nothing else. Every downstream stage reads the redacted text instead; do not pass raw page text to any other stage or quote it into a contract.
