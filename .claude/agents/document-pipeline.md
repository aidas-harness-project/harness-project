...
It performs every mechanical step of Stage 2 in order — checkpoint 1 over each
document, checkpoint 2, segmentation (propose → approve → split) for each
bundle, `classify-only` for each split child, checkpoint 2 for the children,
chunking, and the `page_chunks.json` contract write — and prints a JSON report
naming every phase it ran.

Why this exists rather than you invoking the six tools in sequence: **measured
on CASE_911, that sequence cost 897s of which ~510s (57%) was round trips
between tool calls**, not provider work and not tool startup (a `dao.py`
subprocess is 0.51s; the gaps between spans ran 20-250s). Re-running the same
phases through this driver took 79s, of which 68.2s (86%) is provider calls.
Every step it performs is stated as a RULE in this document, not as a
judgement, so there was nothing for a model to decide between them.
