...
**Checkpoint 1 — OCR + cross-validation + classification.** Run `python tools/run_checkpoint1.py CASE_ID DOC_ID <path to the document under data/raw/> --held-by document-pipeline --run-id RUN_ID`. It wraps `tools/ocr_extract.py`: splits the document into per-page images, runs `reader_a` and `reader_b` as two independent provider calls with no shared context, then runs the configured comparator and records per-page `reading_a`, `reading_b`, `agreement`, and `disagreement_details`. **Dev-phase default: use `--reader-a claude-cli --reader-b claude-cli --comparator claude-cli --classifier-provider claude-cli`** — this is `harness-guardrails-dev`'s documented P8 same-provider weak-P8 fallback; record it honestly in `ocr_result_{document_id}.json` per that skill's instructions (`cross_validation_mode: "single_technology_weak_p8_poc"` etc.). Use this first, before considering the local path.

The fully local path (`--reader-a local-ocr --reader-a-model kor+eng:6 --reader-b local-vlm --reader-b-model qwen3-vl:4b --comparator local-llm --comparator-model qwen3:4b --classifier-provider local-llm --classifier-model qwen3:4b`, after activating `.runtime/local-runtime.env.ps1`) is the eventual target — genuine dual-technology independence (Tesseract vs. an Ollama vision model) — but is **not yet validated on real case documents**: confirmed failing on CASE_003 (`known-gaps.md` item 16) — `local-vlm` (qwen3-vl:4b) returned empty transcriptions on every real page (works only on the local_runtime.py smoke image), and `local-llm` (qwen3:4b) failed checkpoint 2's redaction the same way. `tools/local_runtime.py`'s preflight passing means the runtime loaded, not that the models produce usable output — do not treat preflight success as validation. Do not attempt the local path for a real case run until this is independently re-verified; see `open-decisions.md` #4.
...
**Checkpoint 2 — Redaction.** Run `python tools/redact_document.py CASE_ID DOC_ID --held-by document-pipeline --run-id RUN_ID --provider PROVIDER --model MODEL`. **Dev-phase default: `--provider claude-cli`** (see checkpoint 1's rationale — `local-llm`, qwen3:4b, is confirmed failing on real redaction content as of `known-gaps.md` item 16, returning unparseable reasoning prose instead of JSON). The tool reads each validated page only through `dao.py read-page-text`, strips PII with the configured model, writes the combined `<<<PAGE page=N>>>`-marked text through `dao.py write-redacted-text`, schema-validates `redaction_result_{document_id}.json`, and patches `document_manifest.json` through the DAO. Content redaction alone does not fix a PII-bearing filename — intake already renames raw files to `DOC_XXX`/`GT_XXX` before this checkpoint.

The loopback-only local model (`--provider local-llm`) remains the eventual target for PII exposure reasons (see Access rules below) but should only be used once independently re-verified on real content — do not switch back to it for a real case run based on preflight success alone.
...
- PII exposure remains a risk for external providers. The local-ocr/local-vlm/local-llm configuration removes external transmission from checkpoints 1-2, but only when the project-local runtime is preloaded, `OLLAMA_HOST` remains loopback, and `tools/local_runtime.py` passes.
