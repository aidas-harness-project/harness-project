...
**Checkpoint 1 — OCR + cross-validation + classification.** Run `python tools/run_checkpoint1.py CASE_ID DOC_ID <path to the document under data/raw/> --held-by document-pipeline --run-id RUN_ID`. It wraps `tools/ocr_extract.py`: splits the document into per-page images, runs `reader_a` and `reader_b` as two independent provider calls with no shared context, then runs the configured comparator and records per-page `reading_a`, `reading_b`, `agreement`, and `disagreement_details`. For the recommended fully local run, first activate `.runtime/local-runtime.env.ps1`, then use `--reader-a local-ocr --reader-a-model kor+eng:6 --reader-b local-vlm --reader-b-model qwen3-vl:4b --comparator local-llm --comparator-model qwen3-vl:4b --classifier-provider local-llm --classifier-model qwen3-vl:4b`. Reader A is Tesseract; reader B is a separate Ollama vision-model path, so this is materially more independent than two Tesseract page-segmentation modes. Every local provider requires a preloaded runtime/model, permits loopback only, never downloads during a pipeline run, and never falls back externally. Run `python tools/local_runtime.py` before the stage; any failed preflight check blocks execution.
...
**Checkpoint 2 — Redaction.** Run `python tools/redact_document.py CASE_ID DOC_ID --held-by document-pipeline --run-id RUN_ID --provider local-llm --model MODEL`. The tool reads each validated page only through `dao.py read-page-text`, strips PII with the preloaded loopback-only local model, writes the combined `<<<PAGE page=N>>>`-marked text through `dao.py write-redacted-text`, schema-validates `redaction_result_{document_id}.json`, and patches `document_manifest.json` through the DAO. It never falls back to an external provider. Content redaction alone does not fix a PII-bearing filename — intake already renames raw files to `DOC_XXX`/`GT_XXX` before this checkpoint.
