...
Pass no `--provider` to use the configured default (`llm_providers.DEFAULT_PROVIDER`, currently `openrouter`); `--provider NAME --model SLUG` overrides it. The HTTP providers need a model — from `--model` or `HARNESS_OPENROUTER_MODEL` — and the stage refuses to start without one rather than failing at redaction after the OCR is paid for. `--model` is applied to every provider-backed step in the
stage — the readers, the comparator, the classifier, redaction and the
segmentation judge — so the whole stage runs on one selection rather than on
whatever each role's own env var happens to hold.
...
Every role falls back to the configured default, so name a provider per role
only when you want the readers to DIFFER — `--reader-a NAME --reader-b NAME
--comparator NAME --classifier-provider NAME`, each with its `--*-model`. Two
different models still make a weak P8 (see above); what a per-role flag buys is
that the pair is at least not one model reading twice.
...
**Three non-medical types a liability case turns on** were added 2026-08-21 and are mutually confusable, so the classifier prompt names each explicitly: `insurer_response` is the insurer's covering letter (협조요청, 부지급 통보) even when it summarizes a legal opinion; `legal_opinion` is the reasoned answer itself (법률질의회신서, citing 민법 조문 and ending in "…판단됩니다") **whichever side commissioned it**, since both parties' opinions are evidence and the 수신/발신 block is usually masked; `legal_reference` is published material a party merely attached (위자료 산정기준표, 노동능력상실률 표). Getting these wrong is not cosmetic: before the codes existed the classifier read their titles correctly at 0.90–0.95 confidence, had no bucket but `other`, and claim analysis skipped 24 pages of CASE_053's central evidence with "no medical classification, so no route reaches it".

Once per document — reasoning over its first page's content — the classification pass produces the document-type classification, unless the document arrived pre-flagged (`document_manifest.json`'s `pre_flagged_type`), in which case it trusts the flag and skips inference. Case-wide this is `python tools/run_document_stage.py CASE_ID --checkpoint classify --held-by document-pipeline --run-id RUN_ID`, which the Stage 2 driver runs for you **after** checkpoint 2; it selects the documents that still owe a `document_type`, and skips a bundle still awaiting its split (`segmentation_status: required`) — one label cannot be right for a bundle, which is exactly how CASE_907's 19-page DOC_005 came to be a single `diagnosis_certificate`. The tool writes via the DAO: `ocr_result_{document_id}.json` (`ocr_engine` and `vision_model_name` should record actual provider/model labels and should not imply a dedicated OCR engine unless one was used) and `classification_result_{document_id}.json` — **both one file per document, not a shared file across documents**: `write_contract` overwrites whatever it's given, so if two documents' checkpoint-1 runs both targeted the same flat filename, the second write would silently destroy the first document's record. It then updates this document's `ocr_status`/`ocr_quality`/`cross_validation_status`/`document_type` fields in `document_manifest.json` via `python tools/dao.py patch-manifest-document CASE_ID DOC_ID --fields-file <path to a JSON object of just the fields you're setting> --held-by document-pipeline --run-id RUN_ID` — not `read-contract` + `write-contract`: `document_manifest.json` is a shared file multiple stages update in sequence, and `patch-manifest-document` reads it fresh under the same lock it writes with, instead of assembling a full replacement from a read that happened before the lock was acquired.
