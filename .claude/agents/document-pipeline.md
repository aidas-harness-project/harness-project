...
**Checkpoint 1 — OCR + cross-validation + classification.** Run `python tools/run_checkpoint1.py CASE_ID DOC_ID <path to the document under data/raw/> --held-by document-pipeline --run-id RUN_ID`. It wraps `tools/ocr_extract.py`: splits the document into per-page images, runs `reader_a` and `reader_b` as two independent provider calls with no shared context, then runs the configured comparator and records per-page `reading_a`, `reading_b`, `agreement`, and `disagreement_details`. Provider defaults preserve the old `claude-cli` path for backward compatibility; Codex environments can use the locally authenticated CLI with `--reader-a codex-cli --reader-b codex-cli --comparator codex-cli --classifier-provider codex-cli`, while API providers such as `openai-api` remain available with their required credentials. **Known limitation, not resolved**: two calls to the same provider/model are independent calls but not genuinely different technologies; even two LLM-vision providers are not the same assurance as a traditional OCR engine plus an independent vision read. Do not describe P8 as a true independent-engine cross-check unless the configured providers actually make it one (see `open-decisions.md`).
...
- PII exposure from checkpoint 1's configured provider calls seeing raw, unredacted page images/text is a known, currently-deferred risk (see `open-decisions.md`) — P8 and classification happen before redaction, by necessity. Do not treat it as resolved.
