...
**No `--provider` here on purpose.** The `claude-cli` pin this carried was
removed 2026-08-25, when `openrouter` became `DEFAULT_PROVIDER`: an explicit
flag outranks the environment, so the pin made the harness-wide default
unreachable. Selection falls to `HARNESS_LLM_PROVIDER`, then
`DEFAULT_PROVIDER`. Pass `--provider` only when your briefing names one, and
report which you passed. Because the command no longer shows the transport,
read back what actually ran from `reader_a`/`reader_b` in
`ocr_result_DOC_*.json` rather than assuming it.
