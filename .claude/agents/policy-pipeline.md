...
> **writes `_document_index.json`, which stage 5 uses to link policy clauses** -- skipping
> it does not block stage 5 (the selective driver treats the index as optional), but it
> silently costs the case every clause link: `policy_links` all come back `not_found`
> even when the 약관 is sitting in the case. Dispatching this agent instead was
...
**You do not normalize anything. Clause normalization is RETIRED (2026-08-15)**
— sub-phases 1-4 above describe a contract set that no longer exists, and are
retained only so the pre-2026-08-04 records stay readable. Every policy
document is `text_only_no_normalization`: processed, redacted, chunked and
citable, owing no clause contract. There is nothing for this stage to extract.

Because of that, **this stage is normally not dispatched at all.** The
orchestrator runs `tools/run_policy_pipeline_driver.py`, which records the
manifest fingerprint and finalizes. If you were dispatched anyway, verify via
`document_manifest.json` that every `insurance_policy` document reads
`text_only_no_normalization`, report that the stage is a no-op, and stop. Do
not produce a clause contract, a boundary inventory, or an audit result; the
completion gate no longer asks for them and writing one would create an
artifact nothing verifies.

Why it was retired, measured on the shipped corpus: 211 policy documents
across 4 legacy cases carry `automated_text_pipeline` and **5 clause files
were ever produced** (2.4%), all in CASE_030 — whose policy stage is `failed`
regardless. CASE_112 marked 203 and produced 0, reading `passed` only through
a hand-edited `manual_override` that says so itself. Cost was ~813 provider
calls per 145-page bundle, and the batch never once completed.

And it bought no addressing strength: a normalized clause still carries
`evidence_references: [{document_id, page, quote}]` internally, so the UID
layered on top of source addressing instead of replacing it. `denial-response`
and `claim-analysis` address a clause by `document_id`, page and quoted text,
verified verbatim against the processed source — the same way they always did.
...
Only an **explicit contradiction** blocks the write: a condition whose own
operative predicate says the opposite of its bucket, evidence quotes that
contradict each other, or a condition grounded solely in a quote of the
opposite polarity. Do not repair those by rewording the condition or moving
the bucket — correct the extraction.

A reading the analyzer cannot settle (`mixed`/`ambiguous`) is **not** an error
and needs no human approval. It usually means the operative predicate sits
outside the fragment: `1. 계약자의 고의로 생긴 손해에 대한 배상책임` is a bare
noun phrase whose 보상/부보상 verb lives in the article lead above it. Carrying
the lead into the condition (its text and both source spans) is still the
better extraction and gives a settled reading — but an unsettled one is
recorded and passes, because the buckets have no downstream consumer:
`denial-response` and `claim-analysis` match clauses on
`document_id`/`clause_uid`, source location, and the quoted text, never on
which bucket a condition was filed under.
