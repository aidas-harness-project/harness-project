...
the registered revision your offsets, quotes and UIDs were computed against, per
document (`read-revision-index` prints the current one). A revision recorded before
a re-extraction is stale and the write is refused: re-derive rather than re-stamping.
Legacy (pre-`canonical_v1`) documents keep working unchanged and need neither.
