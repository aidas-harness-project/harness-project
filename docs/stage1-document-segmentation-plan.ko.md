...
### 8. 첫 실물 E2E — 실측 정확도 (구현 7단계)

CASE_025(110p 번들을 DOC_001 하나로 가볍게 인테이크), `propose --grid 4x4
--provider claude-cli`, RUN_20260721_001. 실제 비전 호출 7회 전부 파싱 성공
(파싱 실패 0), 미할당 페이지 0, `needs_full_page` 0, fallback 미포화. 손으로
만든 70경계 베이스라인
(`_segmentation_scratch/CASE_BASELINE_DOC_001/BASELINE.md`) 대비 채점:
...
오류는 베이스라인이 예측한 바로 그 지점 — 반복 양식 구간 — 에 몰렸다:

* **False positive (과분할, 비용 낮음): p2, p21, p23.** p2는 손해사정서
  표지→본문 전환을 새 문서로, p21/p23은 p18–26 의무기록 중간의 비슷한 양식을
  새 시작으로 오인. 시트 보고 사람이 몇 초에 합침.
* **False negative (과병합, 비용 높음): p36, p43, p66–73, p98–100.** 모델이
  반복 양식을 *계속*으로 읽어 경계를 놓침. p36/p43은 제목이 ~7장에 한 번만
  재인쇄되는 세부내역서 앞구간, p66–73은 매 장 재인쇄되는 뒷구간(영수증 8장이
  하나로 병합), p98–100은 영수증 런의 동일 현상. 베이스라인의 "채점에서 가장
  틀리기 쉬운 구간" 주석이 짚은 바로 그 비대칭.

전 구간을 실물로 검증: sheets → 비전 7회 → 채점 → approve → split(60개
DOC_XXX PDF, 번들 superseded 표시, 110페이지 연속·무중복·무구멍 커버) →
**결과 DOC 하나에 checkpoint 1이 실제 dual-path OCR을 돌려 통과**(p1 손해사정서
표지, 두 리더 일치, `other`로 분류). 즉 세그먼테이션 산출물이 기존 파이프라인에
그대로 물린다.

**3×4 시도 — 오히려 더 나쁨, 가설의 정반대.** 동일 번들·provider·베이스라인,
`--grid 3x4` (RUN_20260721_002):

| 격자 | precision | recall | F1 | 세그먼트 | 비전 호출 | needs_full_page |
|---|---|---|---|---|---|---|
| **4×4** | 0.95 | **0.81** | **0.88** | 60 | 7 | 0 |
| 3×4 | 0.90 | **0.50** | 0.64 | 39 | 10 | 3 |

가설은 "셀이 크면 → 작은 글씨 양식을 모델이 잘 읽음 → recall 상승"이었으나
틀렸다: 3×4의 recall이 0.81 → 0.50으로 붕괴, 과병합이 13 → 35로 급증 —
p52–72 세부내역서 뒷구간(매 장 제목, 4×4는 맞힘)과 영수증 런 상당수를 한
세그먼트로 병합했다.

레버는 셀 크기가 아니라 **시트당 페이지 밀도**였다. 4×4는 16장을 나란히 놓아
모델이 인접 페이지를 쉽게 비교해 경계를 잡는데, 3×4의 12장은 "이 반복 양식들은
한 덩어리"쪽으로 기울게 했다. needs_full_page가 (더 큰 셀임에도) 3×4에서만 생긴
것도 같은 신호 — 판단하지 않고 유보했다. 이 corpus의 반복 양식 문제에는 촘촘한
쪽이 낫다.

**격자 4×4로 확정 — 이제 판독성뿐 아니라 정확도 기준으로도(발견 4).** 점수가
훨씬 높고(F1 0.88 vs 0.64) 호출도 더 적다(7 vs 10). p36/p43/p66–73의 과병합
FN은 crop-only 1차 패스의 모델 행동 한계이지 격자로 고쳐지는 게 아니며, 이들에
남은 레버는 full-page fallback이다 — 다음에서 실측.

### 9. Full-page fallback — 과병합의 레버, 실측 (구현 7단계+)

과병합 FN은 모델이 스스로 플래그한 페이지가 아니다(4×4 실행에서 needs_full_page
= 0 — crop 패스가 confidence 0.7로 *확신 있게* 병합함). 그래서 모델의 유보를
트리거로 삼는 fallback은 이들을 못 건드린다. 실제로 먹히는 건 세그먼트 **길이**를
트리거로 삼는 것이다: 병합된 반복 양식 런은 항상 하나의 긴 세그먼트로 나타나므로,
임계값 이상 세그먼트를 페이지별 full-page로 재검토해 과병합된 경계에서 분할한다.
Fail-safe — 판독 불가나 "계속" 판정은 페이지를 연속으로 남기므로, 사람이 검토할
경계를 추가만 할 뿐 조용히 제거하지 않는다.

동일 4×4 CASE_025 실행에서 `refine_long_segments`를 두 임계값으로 실측:

| 실행 | precision | recall | F1 | 과병합 FN | 추가 비전 호출 |
|---|---|---|---|---|---|
| fallback 없음 | 0.95 | 0.81 | 0.88 | 13 | 0 |
| 임계값 5 | 0.94 | 0.91 | 0.93 | 6 | 39 |
| **임계값 4** | 0.93 | **0.96** | **0.94** | **3** | 45 |

임계값 5는 세부내역서 앞구간(p36, p43 — 정확히 정답)과 뒷구간 대부분을 회복했다.
4로 낮추자 임계값 5가 병합으로 남긴 4장짜리 영수증 런(p97–100)까지 끌어들여
p98/99/100을 회복 — 대가는 낮은 confidence 오탐 하나(p26). full-page 패스는 진짜
13장짜리 손해사정서(p2–13)를 올바르게 병합 유지한다(p3만 confidence 0.72로 미끄러짐)
— 즉 진짜 여러 쪽 문서를 함부로 쪼개지 않는다. 남은 3개 FN(p66/71/72)은 full-page
로도 모델이 못 푸는 반복 양식 한계.

**임계값 4로 확정.** recall 0.81 → 0.96, F1 0.88 → 0.94, 이 번들 기준 약 45회 추가
호출(긴 세그먼트 내부 페이지당 1회). `propose --refine`에 배선(기본 off; 지출은
호출자가 선택), 페이지별 verdict 캐시로 재실행·임계값 변경 시 호출한 페이지 재활용.
CLI 경로를 end-to-end로 실행(`propose --refine`, RUN_20260721_003)해 F1 0.94 —
배선된 경로가 실험과 일치함을 확인.

### 10. 두 번째 번들이 드러낸 시트 경계 continuation 누락 — merge 수정

*다른* 케이스(CASE_026, 59p 기왕증·퇴행성 번들)를 두 번째 예시로 돌리자 9페이지
(p33~41)가 `unassigned_pages`로 떨어졌다 — CASE_025엔 없던 일. 원인은 vision 실패도
type enum도 아니었다: p33이 있는 시트의 모델 원본 응답은 **p33을 정확히 읽었고**(자체
evidence에 "unlike the p33-p41 form"), p28에서 시작된 진료비 내역서(입원) 한방 문서의
계속으로 분류했다. 그런데 **시트 경계에서 `continuations` 배열에 p33 적기를 누락**했다
(p34부터 나열). merge는 p33을 "어느 시트도 안 named" → 경계 사례 B로 미할당 처리했고,
그 뒤 구간까지 연쇄로 떨어지며 14장짜리 문서를 찢었다.

`merge_sheet_proposals`에 **continuation 흡수 규칙**으로 수정: 어느 시트도 named하지
않았지만 *두 boundary 사이에 엄격히 위치한* 페이지는 앞 boundary 문서의 내부이므로
그 문서로 흡수한다. 이는 merge의 근본 원리(boundary만 세그먼트를 만들고 시트 경계는
무의미)의 직접적 귀결 — 문서 범위 안의 non-boundary 페이지는 모델이 열거했든 안 했든
그 문서에 속한다. 경계 사례 B의 안전장치는 정확히 보존: *마지막 boundary 뒤*의 구멍은
감쌀 문서가 없으므로 미할당 유지(시트 경계 실수가 아닌 진짜 드롭). 흡수된 페이지는
조용히 삼키지 않고 경고로 표시해 사람이 어느 페이지가 추론으로 붙었는지 본다. 캐시된
응답으로 CASE_026 재-merge: p33~41이 p28~41로 흡수, 미할당 9 → 0. 두 테스트가 양쪽을
고정(boundary 사이는 흡수; 마지막 boundary 뒤 구멍은 미할당 유지).

또한 드러났지만 아직 미해결: 반복 양식 런의 분할/통합 기준이 문서 종류에 따라 다르다 —
영수증·계산서(각 장이 독립 거래, 자체 금액)는 분할, 세부내역서·차트·기록(한 청구/기록이
여러 장, 제목은 반복 헤더)은 통합. CASE_025의 규칙("매 제목 페이지가 별개 문서")과
CASE_026의 필요("내역서 런 통합")가 이 문제의 양면이다. 확정된 방향은 **금액 기반**:
개별 거래 문서는 분할, 기록 문서는 통합. 이를 종류 인식 규칙으로 구현(+ CASE_025 뒷구간
베이스라인 재조정)하는 건 후속 과제로 `known-gaps.md`에 기록.
