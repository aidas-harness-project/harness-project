...
- **번들의 약 절반이 90도 회전되어 스캔되어 있습니다**(p29~73 부근의 긴 구간과
  ...
  **회전 감지기는 넣지 않습니다.** 한때 있었으나 삭제했습니다. 모델이 회전 페이지를
  직접 읽는 이상 그 플래그는 무엇을 막지도, 변형하지도 않고 로그 한 줄만 남기면서도
  보정에는 실제 노력이 들었습니다(임계값을 흔들리는 측정치에 맞춰 두 번 조정했고,
  정상 페이지 오탐은 사람이 결과를 읽고서야 발견됐습니다). **아무도 그에 따라
  행동하지 않는 조건을 감지하는 것은 기능이 아닙니다.**
