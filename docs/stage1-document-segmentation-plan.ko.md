...
- **회전된 페이지가 예외가 아니라 다수: 110장 중 51장(46%).** 전 페이지를 스캔하니
  p21, p29~73, p101~105 구간에 몰려 있습니다. 전 구간 `page.rotation`은 0 —
  콘텐츠 방향이지 PDF 플래그가 아니라 메타데이터로는 알 수 없습니다.

  **원래 계획한 감지기는 작동하지 않으며, 실측으로 확인했습니다.** 잉크 bbox의
  가로/세로 비교는 실패합니다 — 전면 표는 어느 방향으로 스캔했든 페이지를 채우기
  때문입니다. 정상 p1은 348×419, 회전 p41은 372×531로 **둘 다 세로가 길어 둘 다
  정상으로 판정**됐고 감지 건수 0이었습니다.

  **대체 방안(실측):** 행 방향 투영 분산과 열 방향 투영 분산을 비교합니다. 가로
  텍스트는 잉크가 줄 단위로 뭉치므로 아래로 훑으면 밀도가 급격히 진동하고 가로로
  훑으면 평탄합니다. 90도 돌면 둘이 뒤바뀝니다. 110장 전수 측정 결과 회전 페이지는
  최대 1.36, 정상 페이지는 최소 1.62라 임계값 1.5로 잡습니다.

  *이 문서의 이전 기술 정정:* 처음에 "2.99~27.2 대 0.43~0.98, 약 2배 여유"라고
  적었는데 **틀렸습니다.** 정상 페이지를 7장만 표본으로 삼은 결과였고, 실제 여유는
  ±0.14로 훨씬 빠듯합니다. 반례처럼 보였던 케이스(정상 p60이 0.057)는 제가 라벨을
  잘못 붙인 것이었습니다 — p60은 실제로 회전된 진료비 세부내역서였고 렌더링해서
  확인했습니다. **감지기가 아니라 제 라벨이 틀렸습니다.**

- **회전된 페이지는 되돌리지 않고 그대로 둡니다.** 두 실측이 이를 결정했습니다:

  * *어느 방향으로 돌아갔는지 판별하는 것은 실패했습니다.* 양방향으로 돌려 텍스트
    밴드 내 잉크 치우침을 비교했더니 회전이 확실한 9장 중 **3장에서 방향을 오판**했고,
    진짜 정상 페이지 대조군도 0.472~0.551로 흩어져 **기준선 자체가 없었습니다.**
    한 방향으로 찍으면 절반이 뒤집히고, 양방향 수록은 시트가 +57%(회전분만)~
    +100%(전체) 늘어 토큰 절약이라는 설계 목적과 충돌합니다.
  * *모델에게 그대로 읽히는 것은 통합니다.* p65~80(p65~73 회전, p74~80 정상)을 담은
    실제 4×4 시트에 "일부는 90도 회전되어 있다"는 한 줄만 넣어 실제 호출한 결과,
    **회전된 9칸 전부를 읽었고 못 읽은 칸은 0**이었으며, **p74 경계를 confidence
    0.92**로 찾으면서 근거로 "방향과 레이아웃이 급격히 바뀜"을 들었습니다. p65에
    대해서는 confidence 0.45로 낮추고 `needs_full_page`에 올렸는데, 상단 조각으로는
    앞 시트의 p64에서 이어지는지 알 수 없기 때문으로 정확한 판단입니다.

  따라서 `orientation_suspect`는 **정보 제공용**입니다. 사람 검토를 돕고 낮은
  confidence를 설명할 뿐, 무엇을 막거나 변형하지 않습니다.
