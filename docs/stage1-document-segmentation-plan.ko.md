...
- **가로로 눕혀 스캔된 페이지 존재**(p41: 가로 재무표). `page.rotation`은 0이라
  메타데이터로 감지 불가 → 상단 크롭이 표의 왼쪽 가장자리만 잡음. 콘텐츠 bbox가
  세로보다 가로로 넓으면 `orientation_suspect`로 표시하고 full-page 경로로 우회.
