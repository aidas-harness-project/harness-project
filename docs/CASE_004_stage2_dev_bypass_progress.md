...
> **⚠️ 사후 정정 (2026-07-14, 이 문서 최종 결론):** §2·§6이 채택한 해결책 —
> `_OCR_READER_ROLE_FRAMING` 프롬프트 주입 + `CLAUDE.md` "SANCTIONED path" 카브아웃 —
> 은 **방향이 틀렸다.** 그 방어적 문구("이건 인가된 단계다 / 거절하지 마라 / guardrail
> 언급 마라")가 **거절을 막기는커녕 유발**한다: 정상적 OCR 요청은 자기 정당성을 미리
> 변론할 필요가 없으므로, 자식 `claude -p`는 그 방어 어휘 자체를 프롬프트 인젝션 신호로
> 읽고 거절했다. 카브아웃은 그 문구가 만든 문제를 덮으려고 **전역 규범(git 추적, 모든
> 세션·서브에이전트 상속)에 dev 완화를 심는** 대가를 치른 것이다 — §4-4가 스스로 경계한
> 바로 그 위험. `main` 브랜치는 애초에 **순수 중립 프롬프트**("이 이미지를 전사하라")만
> 써서 이 문제가 없었다.
>
> **확정 조치:** `_OCR_READER_ROLE_FRAMING`을 제거하고 `transcribe_image`를 main식 중립
> 프롬프트(`{prompt}\n\nImage: {path}`)로 되돌림. `CLAUDE.md` 카브아웃 삭제. 이 정정에
> 맞춰 `harness-guardrails-dev`의 "P8 same-provider fallback" 섹션과 아래 §들을 갱신.
> **유지된 것:** provider 추상화, 정직한 weak-P8 라벨(`cross_validation_mode`, 스키마
> 필수 유지), `_run`의 cp949 `encoding=` 픽스. 아래 §1~6은 당시 서술을 이력으로 남기되,
> "framing/카브아웃이 refusal을 막는다"는 서술은 이 정정으로 대체된 것으로 읽을 것.
