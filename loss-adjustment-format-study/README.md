...
- Mutating commands assume one cooperative writer on an ordinary local filesystem. The utility does not promise safe concurrent mutation, resistance to a malicious local process or pathname replacement, or transaction recovery after interruption or storage failure. Use a fresh study directory for rebuilds; restart or inspect an interrupted rebuild before reusing it.
