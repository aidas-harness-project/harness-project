...
| 2026-07-29 | Stage 1 contact-sheet output moved from prompt-only JSON requests to a provider-neutral structured-image contract. Claude CLI uses native JSON Schema output; future local providers can implement the same seam. Added exactly-one correction, partial/non-zero halt behavior, and cache binding to prompt/schema/provider/model while retaining failed entries only for diagnostics. | Stage 1 segmentation, provider abstraction, tests | Full-prompt reasoning succeeded but returned prose without JSON systematically |
| 2026-07-30 | Closed a contact-sheet cache-versioning gap (prompt text had changed without a version bump) by bumping to v0.3 and fingerprinting the prompt text itself into the cache contract. Remeasured CASE_025's 110-page bundle for real: cap-ratio 1.0/no-refine scored F1 0.8589 (R 1.0000); adding `--refine` improved to F1 0.9150 -- the best measured for this bundle, beating the pre-structured-output baseline (F1 0.8769). | tools/segment_case.py, Stage 1 plan | Prompt and version constant had drifted once already |
