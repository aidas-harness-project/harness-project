...
def captured(monkeypatch):
    ...
    # Two parameters, not one: `reclassify` arrived on the selective-lane side
    # of this merge while these tests were written against the one-argument
    # form. A stub narrower than the function it replaces fails as a TypeError
    # inside the code under test, which reads as a defect in that code.
    monkeypatch.setattr(run_stage2, "unclassified_children",
                        lambda manifest, reclassify=None: [])
...
def test_the_model_reaches_split_child_classification(captured, monkeypatch):
    monkeypatch.setattr(run_stage2, "unclassified_children",
                        lambda manifest, reclassify=None: [{"document_id": "DOC_002"}])
