...
| `DenialResponseAgent` | 반려사유 추출, taxonomy 분류, 약관 매칭, **반박 포인트 생성** | 가명처리된 안내문 텍스트 + `normalized_policy_clause.json` + `denial_validation_result.json` | **`denial_reason_result.json`**, `policy_to_denial_matching_result.json`, `rebuttal_points.json`·`.md` |
...
| `CriticEvaluationAgent` | 금지 표현 탐지, 평가, 실패 유형 기록 | 초안 + 정답지 (`data/ground_truth/`, 평가 전용) | `critic_result.json`, `expert_review.json`, `evaluation_result.json`, `evaluation_summary.json` |

모든 컴포넌트 출력은 [컴포넌트 표준 출력 계약](templates/component-output.md)을
따른다. 묶음 Agent의 실행 정의는 `.claude/agents/`, 실행 순서·게이트는
오케스트레이터 스킬(`.claude/skills/loss-adjustment-pipeline/`)에 있다
(2026-07-07 하네스 구축 — 반박 포인트 생성은 denial 도메인 연속성을 위해
`DenialResponseAgent` 소속으로 확정).
