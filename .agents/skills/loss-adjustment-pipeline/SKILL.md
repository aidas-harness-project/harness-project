...
At the end of a run (or when halted), report to the user: per-stage pass/fail/pending status from `_run_state.json`, validation PASS/FAIL/SKIP tally, the `review_required` count and routing, any partial/warning list, and next actions (e.g. awaiting human review). Ask for feedback — this harness evolves from it, see the root `CLAUDE.md` changelog.

Get the `review_required` half from `python tools/dao.py collect-review-flags CASE_ID` — **this is P3's aggregation point, and it is the report's job to be it.** P3 says a flagged claim is hedged and flagged but "does not halt the current stage", and that flagged claims "surface together at the aggregation/report stage for batch human review". Until this command existed the flagging half ran and the surfacing half did not: nothing read the field, so CASE_909 finished carrying 18 flags that could only be seen by opening eighteen files by hand. Do not hand-assemble this list — reading the contracts yourself is what the command replaces.

Read the output as a report, not a task list. It is read-only and exits 0 with flags wide open, by design: **a flag is not a defect.** Most record a limit of the material — a date blanked by redaction is not recoverable by retry (`retry_would_not_help`), and asking a reviewer to confirm it costs their attention for nothing. Relay `unresolved_by_reviewer_role` so the batch splits by who can actually answer (손해사정사/의사/법률전문가), carry each entry's own `warnings` (the agent's stated reason is the content a reviewer needs; a bare filename list is not), and say plainly which few genuinely turn on a human judgment versus which record an absence. `resolved` entries are already closed by a gate — do not re-raise them. A non-empty `unreadable` (exit 1) means the sweep itself could not be trusted; surface that as a failure, never as a clean pass.
