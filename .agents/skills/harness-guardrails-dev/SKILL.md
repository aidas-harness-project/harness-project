...
The switch condition is **"local-ocr/local-vlm validated against representative real case documents,"** not merely "provisioned" or "preflight passes." `tools/local_runtime.py` passing only means the runtime loaded — it says nothing about output quality. This distinction is not theoretical: CASE_003 (`known-gaps.md` item 16) confirmed the old hybrid-thinking `qwen3-vl:4b` and `qwen3:4b` tags failed real document work despite a clean preflight. The explicit Instruct Q4 replacements passed one DOC_013 checkpoint-1 page on 2026-07-16, but one page does not satisfy the multi-type v1 matrix in `docs/ocr-improvement-roadmap.md`, and local redaction remains unvalidated. Until that matrix passes, default to claude-cli and use the local pair only for explicitly scoped validation runs.
