...
- Provider-based P8 does not automatically mean genuinely different extraction technologies. The recommended `local-ocr` + `local-vlm` pair uses different technologies; two `local-ocr` calls with different page-segmentation modes remain a weaker fallback and must not be described as equivalent. See `open-decisions.md`.
