...
| 2 | Document Processing | `document-pipeline` | One top-level stage, 3 internal DAO checkpoints: (a) provider-backed OCR + P8 dual-path cross-validation + classification, (b) provider-backed redaction through `tools/redact_document.py`, (c) deterministic chunking. A whole-document photograph/visual-evidence input may be human-resolved as `non_text_image`: no transcription/classification quote/redaction/chunk is fabricated, and it is recorded as `expert_review_only` plus an explicit `page_chunks.json` exclusion. Recommended offline text runs pair preinstalled Tesseract (`local-ocr`) with a preloaded loopback Ollama vision reader (`local-vlm`), then use `local-llm` for comparison/classification/redaction; missing local dependencies fail closed with no external fallback. |
