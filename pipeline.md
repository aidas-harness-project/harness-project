...
| 4 | Policy Clause Processing | `policy-pipeline` | Boundary inventory → exact source-span accounting → extraction → normalization mapping. Every non-whitespace policy-source character is covered or explicitly excluded; unresolved/missing boundaries block finalization. |
