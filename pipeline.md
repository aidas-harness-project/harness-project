...
| 4 | Policy Clause Processing | `policy-pipeline` | Boundary inventory → exact source-span accounting → semantic classification → normalization mapping. Every non-whitespace policy-source character is covered or explicitly excluded; unresolved/missing boundaries block finalization. Immutable `clause_uid`/`condition_uid` values are downstream join keys, while sequential `clause_id` is display-only. Clause kinds use dedicated buckets so obligations, definitions, procedures, termination, disputes, and coverage-start rules cannot be mistaken for payout conditions. Condition evidence is labeled direct/composite/insufficient; heading-only and lexically unrelated citations fail a deterministic support floor, while composite support is explicitly review-gated. When a large policy PDF is one physical parent carved into segments (e.g. CASE_030's 240-page 약관: parent `DOC_001` split into per-특약/조항 segments + a 별표 appendix segment), two shapes are exempt from the per-document normalization requirement: a **segmented physical parent** (any doc that is another doc's `source_document_id`) is normalized *through* its segments, so its completeness is instead governed by a `policy_parent_coverage_{id}.json` that accounts for every logical page 1..N exactly once (owned-by-segment / reference-table / administratively-excluded-with-reason — no gap, dup, or unresolved page); a **reference-table-only segment** (별표 tables in `reference_table_{id}.json`, not clauses) is waived the normalized-clause + audit requirement but still owes a complete boundary inventory. Every other segment still owes normalized clauses + boundary inventory + audit. Decision-bearing appendices live in `reference_table_{id}.json` with immutable source-derived table/row/cell UIDs, each cell checked against its page and quote; normalized clauses link to tables/rows by UID, never by array position or display label. Each normalizing document also has a `policy_audit_result_{id}.json` bound to the exact SHA-256 of its normalized clause + boundary inventory + optional reference table — rewriting any audited contract makes the audit stale, and the stage cannot finalize with a missing/stale audit or an open finding; downstream coverage/requirement/denial references resolve only against the current clear audit. |
