...
| # | Stage | Agent | Notes |
|---|---|---|---|
| 1 | Case Intake & Document Segmentation | (`tools/intake_case.py`, `tools/segment_case.py`) | D2-gated intake: every file gets a `_source_ledger.json` entry; a single rejection blocks the case. Every intaken PDF then starts `segmentation_status: pending_review` until a genuine human records `required` or `not_required` through the DAO. Automatic bundle classification is not implemented. **No OCR here -- output is document STRUCTURE, not text.** A required bundle is **OCR'd before it is split** (`run_checkpoint1.py --bundle-ocr`, OCR only — no classification, since `document_type` is per-document and one label cannot fit a bundle) and redacted, so boundaries come from real page text rather than a downscaled crop; `propose` resolves evidence in the order processed text → the PDF's embedded layer → vision. `split_bundle` then redistributes the bundle's pages to its children, renumbered from 1 with P8 verdicts intact, so children are never re-OCR'd. Only a bundle with no processed text falls through to 4×4 top-third sheets → boundary proposal → automatic targeted full-page fallback → optional long-segment `--refine`. Either way: human range approval → split into logical `DOC_XXX.pdf`, retaining the bundle as `superseded_bundle`. Each materialized child owns a real `data/raw/.../DOC_XXX.pdf`, so its manifest `document_role` is `physical`; `source_file_name` and `source_page_start`/`source_page_end` retain bundle provenance but do not make it a raw-less processed-text `segment` with `source_document_id`/`page_map`. Stage 2 is structurally gated: `run_checkpoint1.py` performs a case-wide DAO preflight before provider construction/PDF access and returns `blocked_segmentation` if any PDF is pending, required-but-unsplit, a legacy PDF with no status, or the target is a superseded bundle. A provisional type is never trusted downstream. Pipeline execution remains agentic through the skill; there is no standalone runner. |
