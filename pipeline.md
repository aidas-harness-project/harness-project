...
| # | Stage | Agent | Notes |
|---|---|---|---|
| 1 | Case Intake & Document Segmentation | (`tools/intake_case.py`, `tools/segment_case.py`) | D2-gated intake: every file gets a `_source_ledger.json` entry (pending → human sets approved/rejected); a single rejected file blocks the whole case. Then segmentation: a raw source is a *bundle* concatenating several logical documents (claim form, diagnosis certificate, medical records, receipts, insurer response) into one PDF, so before any OCR it is split into logical documents. **No OCR here -- the output is document STRUCTURE, not text** (`segmentation_proposal_{DOC}.json`, schema pins `ocr_performed` to const false). Flow: render low-res contact sheets → a vision provider proposes boundaries (`--refine` re-examines long merged segments full-page to recover over-merged boundaries; measured F1 0.94 on the 110p bundle) → **human approves the proposal (a gate that blocks stage 2)** → split into per-document `DOC_XXX.pdf`, the bundle entry marked `superseded_bundle`. A `provisional_document_type` may be guessed from a cropped cell but is NEVER trusted downstream -- checkpoint 1 owns real classification against OCR'd text. |
