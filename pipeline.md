...
| # | Stage | Agent | Notes |
|---|---|---|---|
| 1 | Case Intake & Document Segmentation | (`tools/intake_case.py`, `tools/segment_case.py`) | D2-gated intake: every file gets a `_source_ledger.json` entry (pending → human sets approved/rejected); a single rejected file blocks the whole case. The orchestrator/operator then identifies multi-document raw bundles (automatic candidate detection is not implemented). **No OCR here -- the output is document STRUCTURE, not text** (`segmentation_proposal_{DOC}.json`, schema pins `ocr_performed` to const false). Flow: render low-res 4×4 top-third contact sheets → propose boundaries → automatically recheck crop-ambiguous `needs_full_page` pages full-page under the 25% cap → optionally `--refine` long, confidently over-merged segments → **human approves/corrects every range (a gate that blocks stage 2)** → split into per-document `DOC_XXX.pdf`, retaining the bundle as `superseded_bundle`. Full-page rule: own/repeated title means a new document; no title means continuation; unreadable stays pending for human review. A `provisional_document_type` is never trusted downstream -- checkpoint 1 owns classification against OCR'd text. Pipeline execution is agentic through the `loss-adjustment-pipeline` skill; there is no standalone runner. |
| 2 | Document Processing | `document-pipeline` | One top-level stage, 3 internal DAO checkpoints: (a) provider-backed OCR + P8 dual-path cross-validation + classification (a plain-text source takes a deterministic embedded-text decode instead, `cross_validation_mode: deferred_poc`), (b) provider-backed redaction through `tools/redact_document.py` (via the `tools/redaction.py` Redactor abstraction, with per-page fidelity verification), (c) deterministic chunking. A whole-document photograph/visual-evidence input may be human-resolved as `non_text_image`: no transcription/classification quote/redaction/chunk is fabricated, and it is recorded as `expert_review_only` plus an explicit `page_chunks.json` exclusion. All P8 readers are LLM-vision-backed (claude-cli / codex-cli / openai-api), so any reader pair is a documented weak P8 (`single_technology_weak_p8_poc`); a genuinely technology-independent reader (a real OCR engine) is deferred -- see `open-decisions.md` #4. |
